...
import { normalizeBasePath, TAB_GROUPS, localizedGroupLabel, subtitleForTab, titleForTab } from "./navigation.ts";
import { t, getLocale, setLocale, type Locale } from "./i18n.ts";
...
export function renderApp(state: AppViewState) {
  ...
  const chatDisabledReason = state.connected ? null : t("Disconnected from gateway.");
    ...
    <div class="shell ${isChat ? "shell--chat" : ""} ${chatFocus ? "shell--chat-focus" : ""} ${state.settings.navCollapsed ? "shell--nav-collapsed" : ""} ${state.onboarding ? "shell--onboarding" : ""}">
      <header class="topbar">
        ...
        <div class="topbar-status">
          ...
          <button
            class="pill"
            style="cursor:pointer;border:none;background:var(--bg-2);font-size:0.8rem;"
            @click=${() => {
              const next: Locale = state.settings.locale === "zh" ? "en" : "zh";
              setLocale(next);
              state.applySettings({ ...state.settings, locale: next });
            }}
            title="${t("Language")}"
            aria-label="${t("Language")}"
          >🌐 ${state.settings.locale === "zh" ? "EN" : "中文"}</button>
        </div>
      </header>
